...
async fn main() {
    ...
    let rezult2 = graphql_parser::parse_query(stri2.as_str());
    match rezult2 {
        Ok(ast) => println!("{:?}", ast),
        Err(e)  => println!("{:?}", e)
    }
    ...
    let rezult3 = graphql_parser::parse_query(stri3.as_str());
    match rezult3 {
        Ok(ast) => println!("{:?}", ast),
        Err(e)  => println!("{:?}", e)
    }
    ...
    let rezult4 = graphql_parser::parse_query(stri4.as_str());
    match rezult4 {
        Ok(ast) => println!("{:?}", ast),
        Err(e)  => println!("{:?}", e)
    }
    ...
    let rezult5 = graphql_parser::parse_query(stri5.as_str());
    match rezult5 {
        Ok(ast) => println!("{:?}", ast),
        Err(e)  => println!("{:?}", e)
    }
    ...
    let rezult6 = graphql_parser::parse_query(stri6.as_str());
    match rezult6 {
        Ok(ast) => println!("{:?}", ast),
        Err(e)  => println!("{:?}", e)
    }
    ...
    let rezult7 = graphql_parser::parse_query(stri7.as_str());
    match rezult7 {
        Ok(ast) => println!("{:?}", ast),
        Err(e)  => println!("{:?}", e)
    }

    let stri8 = String::from("{field1(p1:1,p2:\"as              d              \"){subf1 subf2(p3:0){s}}}");
    let option = graphql_parser::parse_query(stri8.as_str());
    ...
    let stri9 = String::from("  {    field1 (    p1 : 1   ,  p2    :   \"asd\" ) {    subf1   subf2   (    p3 :  0 )   {   s  }  }    } ");
    let option2 = graphql_parser::parse_query(stri9.as_str());
