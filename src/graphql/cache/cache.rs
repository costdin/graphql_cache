...
pub struct Cache<K: 'static + Hash + Eq + Send + Sync, T: 'static + Sync + Send> {
...
impl<K: 'static + Hash + Eq + Send + Sync, T: 'static + Sync + Send> Drop for Cache<K, T> {
    fn drop(&mut self) {
        println!("Dropping");
...
impl<K: 'static + Hash + Eq + Send + Sync, T: 'static + Sync + Send> Cache<K, T> {
