...
impl<'a> Iterator for Tokenizer<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            match iterator.next() {
                Some((pos, c)) if !self.in_quotes && (TOKEN_BREAKER.contains(&c) || is_insignificant_character(c)) => {
                    ...
                    if pos > start {
                        ...
                        match iterator.next() {
                            Some((_, '.')) => match iterator.next() {
                                Some((_, '.')) => { self.slice = &self.slice[start+3..]; return Some("..."); },
                                _         => { self.slice = &self.slice[start+2..]; return Some(".."); },
                            },
                            _         => { self.slice = &self.slice[start+1..]; return Some("."); }
                        }
